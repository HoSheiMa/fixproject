...
class CoursesVideoList extends Component {
    ...
    get_recent_course = () => {
        ...
        const ca_key1 = this.props.ca_key1;
        const co_key1 = this.props.co_key1;
        const ca_key2 = this.props.ca_key2;
        const co_key2 = this.props.co_key2;
        if (co_key1 == '') {
            Firebase.database().ref('User_info/' + this.props.userkey).update({
                ca_key1: ca_id,
                co_key1: co_id,
            })
            self.props.course_select({
                ca_key1: ca_id,
                co_key1: co_id,
                ca_key2: '',
                co_key2: '',
            ...
            if (co_key2 == '') {
                ...
                if (co_key2 != co_id) {
                    Firebase.database().ref('User_info/' + this.props.userkey).update({
                        ca_key1: self.props.ca_key2,
                        co_key1: self.props.co_key2,
                        ca_key2: ca_id,
                        co_key2: co_id,
                    })
                    self.props.course_select({
    ...
    _rendermakesession({ item, index }) {
        ...
        return (
            <View style={styles.sessioncontainer}>
                <TouchableOpacity onPress={() => { navigate('CoursesVideoDetail', { videoId: item.key }) }}>
                ...
                <View style={styles.sessiontextarea}>
                    <Text style={styles.sessiontittle}>{this.formatTextEclips(item.name, 15)}</Text>
                    ...
                    <Text style={styles.sessiondescription}>{this.formatTextEclips(item.description, 70)}</Text>
                </View>
            </View>
        );
    }
...
export default connect(mapStateToProps, mapDispatchToProps)(CoursesVideoList);
